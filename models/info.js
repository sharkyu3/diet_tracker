...
module.exports = (dbPoolInstance) => {
  ...
  let getAllMeals = (callback, ecoId) => {
    let query = 'SELECT meals.id, meals.title, meals.photo_URL, meals.creation_info, meals.cheat, users.username, ecosystems.group_name, ecosystems.description FROM meals INNER JOIN users ON (meals.users_id = users.id) INNER JOIN ecosystems ON (ecosystems.id = users.ecosystems_id) WHERE ecosystems.id = $1';
    let values = [ecoId];
  ...
  let getAllExercises = (callback, ecoId) => {
    let query = 'SELECT exercises.id, exercises.type, exercises.duration, exercises.creation_info, exercises.photo_URL, users.username FROM exercises INNER JOIN users ON (exercises.users_id = users.id) INNER JOIN ecosystems ON (ecosystems.id = users.ecosystems_id) WHERE ecosystems.id = $1';
    let values = [ecoId];
